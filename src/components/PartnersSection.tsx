...
const PartnersSection = () => {
  ...
  const partners = [
    { logo: "ECB", nameKey: "partners.ecb.name", descKey: "partners.ecb.desc" },
    { logo: "BNB", nameKey: "partners.binance.name", descKey: "partners.binance.desc" },
    { logo: "COINW", nameKey: "partners.coinw.name", descKey: "partners.coinw.desc" },
  ];
    ...
    <section id="partners" className="py-20 lg:py-32 px-6 lg:px-12" style={{ background: "hsl(270 100% 6%)" }}>
      <div ref={ref} className="max-w-7xl mx-auto opacity-0">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-6 max-w-4xl mx-auto">
          {partners.map((p) => (
            <div key={p.logo} className="card-glass rounded-2xl p-8 text-center hover:-translate-y-1 hover:border-primary/50 transition-all duration-300 glow-box-hover">
              <div className="w-16 h-16 rounded-xl gradient-primary flex items-center justify-center mx-auto mb-4">
                <span className="font-display text-lg text-foreground tracking-wider">{p.logo}</span>
              </div>
